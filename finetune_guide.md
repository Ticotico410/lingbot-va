...
```
/home/karthus_chen/unitree_sh_disk/tools/ycb/datasets/uniarml1/pick_key_and_controller_335/lerobot_v2.1
```
...
改 `script/run_va_posttrain.sh` 里的 `WANDB_*`，或训练 cfg 里设 `enable_wandb=False`。
...
- resize ≈ **256×256**，目标采样 **~7.5–15 fps**（建议 10fps）
...
- 仓库**无**官方抽 latent 脚本，需自写或参考 [Wan-Video](https://github.com/Wan-Video)；字段 schema 见 README；示例数据：[Issue #29](https://github.com/Robbyant/lingbot-va/issues/29)

### 2.3 生成 `empty_emb.pt`

放在 `dataset_path` 下（CFG dropout 用）。用 ckpt 的 UMT5 对空串编码后 `torch.save`：

```python
# 伪代码：load tokenizer + text_encoder → encode("") → save empty_emb.pt
```
...
| 字段 | 建议值 | 说明 |
|---|---|---|
| `wan22_pretrained_model_name_or_path` | `/path/to/lingbot-va-base` | 上一步下载路径 |
...
```python
dataset_path = '.../pick_key_and_controller_335/lerobot_v2.1'  # 会递归找 info.json
empty_emb_path = os.path.join(dataset_path, 'empty_emb.pt')
learning_rate = 1e-5
batch_size = 1
gradient_accumulation_steps = 8   # 有效 batch≈ NGPU*8；资源够可加大
num_steps = 20000                 # 334 ep，可按验证调
save_interval / cfg_prob / load_worker  # 跟 demo_train 即可
```
...
```bash
NGPU=8 CONFIG_NAME='uniarm_train' bash script/run_va_posttrain.sh
# 断点：在 train cfg 加 resume_from='/path/to/train_out/.../checkpoint_step_xxx'
```
